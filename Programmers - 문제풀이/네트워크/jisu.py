...
def bfs(initComputer:int):
    visited[initComputer] = True                    # 초기 컴퓨터 방문 처리
    queue: Deque[int] = deque([initComputer])

    while queue:
        curComputer = queue.popleft()

        for computer in networks[curComputer]:      # 현재 탐색 중인 컴퓨터와 연결된 컴퓨터들을 탐색
...
def solution(n: int, computers: List[List[int]]) -> int:
    ...
    for computer in range(n):               
        if not visited[computer]:
            numOfNetworks += bfs(computer)  # 정상적인 탐색이 한 번 끝나면 하나의 네트워크가 형성되어 있다는 것임
...
def bfs_adjMatrix(initComputer: int, computers: List[List[int]]) -> int:
    global visited
    
    queue = deque([initComputer])
    visited[initComputer] = True
    
    while queue:
        curComputer = queue.popleft()       
        for nxtComputer, is_connect in enumerate(computers[curComputer]):   # 현재 탐색 중인 컴퓨터의 연결 정보 순회
            if is_connect and not visited[nxtComputer]:                       # 연결되어있고 아직 방문되지 않은 컴퓨터만 탐색
                queue.append(nxtComputer)
                visited[nxtComputer] = True           
    return 1
...
def solution_adjMatrix(n: int, computers: List[List[int]]) -> int:
    ...
    for computer in range(n):
        if not visited[computer]:
            numOfNetworks += bfs_adjMatrix(computer,computers)
