...
- low, high = 0, N-1로 두고, 둘 중 홀수 더 가까운 쪽의 인덱스를 업데이트
...
- 이러면 홀수 개수를 매번 파악할 필요 없이, 포인터를 움직일때 홀수 개수를 업데이트 하는 방법으로 홀수 개수 관리 가능
- 현재 range의 홀수 개수가 K 이하인경우
    - 범위를 더 넓혀서 탐색할 수 있음, high를 넓혀서 탐색 -> 홀수 개수 변화에 유의
    - 범위가 넓어지는 경우 최대 range의 길이 업데이트 필요(홀수 개수 제외)
- 현재 range의 홀수 개수가 K 초과인 경우
    - 범위를 좁혀서 탐색할 수 있음, low를 증가시켜 탐색 -> 홀수 개수 변화에 유의
...
while True:
    if odd_cnt <= K:  # 현재 range의 홀수 개수가 K 이하인경우 범위가 넓어짐
        ...
        if S[high] % 2 == 1:  # 이동 후 포인터의 인덱스 값이 홀수이면
            odd_cnt += 1  # 홀수 개수 증가
    else:  # 현재 range의 홀수 개수가 K 초과인경우 범위가 좁아짐
